...
namespace PayAtTable.Server.DemoRepository
{
    public class TablesRepositoryDemo: ITablesRepository
    {
        ...
        public IEnumerable<Table> GetTablesWithOrders()
        {
            var occupied = from t in SampleData.Current.Tables
                           where SampleData.Current.Orders.Exists(m => m.TableId.Equals(t.Id))
                           select t;
